...
public class RevertUnchangedAssetsToPreviousAssetState
{
    ...
    public static ReturnCode Run(IAddressableAssetsBuildContext aaBuildContext, ContentUpdateContext updateContext)
    {
        ...
        if (updateContext.ContentState.cachedBundles == null)
            UnityEngine.Debug.LogWarning($"ContentUpdateContext does not contain previous asset bundle info, remote static bundles that are updated will not be cacheable.  If this is needed, rebuild the shipped application state with the current version of addressables to update the addressables_content_state.json file.  The updated addressables_content_state.json file can be used to create the content update.");
